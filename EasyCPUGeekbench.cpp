...
void MBM()
{
	...
	long long quyu=(2*R)/num;
	for(int i=1;i<num;i++)
	{
		thread TD(ThreadWork,-R+quyu*(i-1),-R+quyu*i,limOfThread,i,num);
		TD.detach();
	}
	ReadyCount++;
	while(ReadyCount!=num);
	long long TickA = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
	Calc(R-quyu,R,limOfThread,0);
	cout<<"It`s Running!\n";
	ReadyCount--;
//	while(ReadyCount>0);
	long long TickB = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
	char Text[1024]={0};
	sprintf_s(Text,1024,"Used time=%d,TickA=%lld,TickB=%lld,Your Score=%lld\n",TickB-TickA,TickA,TickB,Limit/(TickB-TickA));
	cout<<Text;
	system("pause");
}
